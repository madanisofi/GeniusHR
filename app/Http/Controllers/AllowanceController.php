...
class AllowanceController extends Controller
{
    ...
    public function store(Request $request)
    {
        if (Auth::user()->can('Create Allowance')) {
            ...
            return back()->with('success', __('Allowance  successfully created.'));
        } else {
    ...
    public function update(Request $request, Allowance $allowance)
    {
        if (Auth::user()->can('Edit Allowance')) {
            if ($allowance->created_by == Auth::user()->creatorId()) {
                ...
                return back()->with('success', __('Allowance successfully updated.'));
            } else {
                return back()->with('error', __('Permission denied.'));
            }
        } else {
    ...
    public function destroy(Allowance $allowance)
    {
        ...
        if (Auth::user()->can('Delete Allowance')) {
            if ($allowance->created_by == Auth::user()->creatorId()) {
                ...
                return back()->with('success', __('Allowance successfully deleted.'));
            } else {
                return back()->with('error', __('Permission denied.'));
            }
        } else {
