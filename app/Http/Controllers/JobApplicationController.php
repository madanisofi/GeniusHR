...
class JobApplicationController extends Controller
{
    ...
    public function addSkill(Request $request, $id)
    {
        if (Auth::user()->can('Add Job Application Skill')) {
            ...
            return back()->with('success', __('Job application skill successfully added.'));
        } else {
            return redirect()->route('job-application.index')->with('error', __('Permission denied.'));
        }
    }
    ...
    public function addNote(Request $request, $id)
    {
        if (Auth::user()->can('Add Job Application Note')) {
            ...
            return back()->with('success', __('Job application notes successfully added.'));
        } else {
            return redirect()->route('job-application.index')->with('error', __('Permission denied.'));
        }
    }
    ...
    public function destroyNote($id)
    {
        if (Auth::user()->can('Delete Job Application Note')) {
            ...
            return back()->with('success', __('Job application notes successfully deleted.'));
        } else {
            return redirect()->route('job-application.index')->with('error', __('Permission denied.'));
        }
    }
    ...
    public function jobBoardConvertData(Request $request, $id)
    {
        ...
        if ($setings['employee_create'] == 1) {
            ...
            return back()->with('success', __('Application successfully converted to employee.') . (isset($smtp_error) ? $smtp_error : ''));
        }

        return back()->with('success', __('Application successfully converted to employee.'));
    }
