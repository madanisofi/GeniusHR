...
class OvertimeController extends Controller
{
    ...
    public function store(Request $request)
    {
        if (Auth::user()->can('Create Overtime')) {
            ...
            return back()->with('success', __('Overtime  successfully created.'));
        } else {
    ...
    public function update(Request $request, $overtime)
    {
        ...
        if (Auth::user()->can('Edit Overtime')) {
            if ($overtime->created_by == Auth::user()->creatorId()) {
                ...
                return back()->with('success', __('Overtime successfully updated.'));
            } else {
                return back()->with('error', __('Permission denied.'));
            }
        } else {
    ...
    public function destroy(Overtime $overtime)
    {
        if (Auth::user()->can('Delete Overtime')) {
            if ($overtime->created_by == Auth::user()->creatorId()) {
                ...
                return back()->with('success', __('Overtime successfully deleted.'));
            } else {
                return back()->with('error', __('Permission denied.'));
            }
        } else {
