...
class ReportController extends Controller
{
    ...
    public function monthlyAttendance(Request $request)
    {
        if (Auth::user()->can('Manage Report')) {
            ...
            $employeesAttendance = [];
            $totalPresent        = $totalLeave = $totalEarlyLeave = $totalAbsence = 0;
            ...
            foreach ($employees as $id => $employee) {
                ...
                foreach ($dates as $date) {
                    ...
                    if ($dateFormat <= date('Y-m-d')) {
                        ...
                        } else if (!empty($employeeAttendance) && $employeeAttendance->status == 'Absence') {
            ...
            $data['totalAbsence']    = $totalAbsence;
    ...
    public function employeeRoom(Request $request)
    {
        if (Auth::user()->can('Manage Report')) {
            $RoomEmployee = Employee::selectRaw('d.name as room, count(employees.id) as count')->rightJoin('designations as d', 'employees.designation_id', '=', 'd.id')->where('d.created_by', Auth::user()->creatorId())->groupBy('designation_id')->get();
            return view('report.room', compact('RoomEmployee'));
        } else {
            return back()->with('error', __('Permission denied.'));
        }
    }
    ...
    public function employeePosition(Request $request)
    {
        ...
        if (Auth::user()->can('Manage Report')) {
            ...
            $position = Position::where('created_by', $created_by)->get();
            $employee = Employee::where('created_by', $created_by)->get();
            $employeetype = EmployeeType::where('created_by', $created_by)->get();
            $education = Educations::where('created_by', $created_by)->orderBy('name', 'ASC')->get();
            $employeeData = Employee::where('created_by', $created_by)->get()->toArray();


            $data = [];
            $header = ['Position', 'Total', 'Male', 'Female'];

            foreach ($employeetype as $type) {
                array_push($header, $type->name);
            }

            foreach ($education as $edu) {
                array_push($header, $edu->name);
            }
            ...
            foreach ($position as $pos) {
                ...
                foreach ($employee as $emp) {
                    if ($emp->position_id == $pos->id) $total++;
                    if ($emp->gender == 'Male' && $emp->position_id == $pos->id) $male++;
                    if ($emp->gender == 'Female' && $emp->position_id == $pos->id) $female++;
                }

                $data1 = [$pos->name, $total, $male, $female];

                // employee type
                foreach ($employeetype as $type) {
                    $data2[] = $this->search($employeeData, array('position_id' => $pos->id, 'employeetype_id' => $type->id));
                }

                // education
                foreach ($education as $edu) {
                    $data3[] = $this->search($employeeData, array('position_id' => $pos->id, 'education_id' => $edu->id));
                }
            ...
            return view('report.position', compact('data', 'header'));
        } else {
            return back()->with('error', __('Permission denied.'));
        }
    }
