...
class SettingsController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (Auth::user()->type == 'company' || Auth::user()->type == 'super admin') {
            ...
            if ($request->logo_light) {
                $request->validate(['logo_light' => 'image|mimes:png',]);
            ...
            if (!empty($request->title_text) || !empty($request->footer_text) || !empty($request->default_language) || isset($request->display_landing_page) || isset($request->disable_signup_button) || !empty($request->theme_color) || !empty($request->cust_theme_bg) || !empty($request->cust_darklayout)) {
                $post = $request->all();
    ...
    public function savePaymentSettings(Request $request)
    {
        ...
        if (Auth::user()->type == 'company' || Auth::user()->type == 'super admin') {
            ...
            return back()->with('success', __('Payment successfully updated.'));
        } else {
            return back()->with('error', 'Permission denied.');
        }
    }
    ...
    public function saveCompanySettings(Request $request)
    {
        if (Auth::user()->type == 'company' || Auth::user()->type == 'super admin') {
            ...
            if (isset($request->working_days)) {
                $post['working_days'] = json_encode($request->working_days);
            } else {
                $post['working_days'] = json_encode([]);
            }
            ...
            if (!isset($request->automatic_basic_salary)) {
                $post['automatic_basic_salary'] = 'off';
            }

            if (!isset($request->access_restrictions)) {
                $post['access_restrictions'] = 'off';
            }
            ...
            if (!isset($request->qr_presence)) {
                $post['qr_presence'] = 'off';
            }

            if (!isset($request->auto_expense_payroll)) {
                $post['auto_expense_payroll'] = 'off';
            }

            if (!isset($request->tiered_leave)) {
                $post['tiered_leave'] = 'off';
            }

            if (!isset($request->late_fee_calculation)) {
                $post['late_fee_calculation'] = 'off';
            }

            if (!isset($request->presence_mode_with_distance)) {
                $post['presence_mode_with_distance'] = 'off';
            }

            if (!isset($request->selfie_presence)) {
                $post['selfie_presence'] = 'off';
            }

            if (!isset($request->open_and_close_presence)) {
                $post['open_and_close_presence'] = 'off';
            }

            if (!isset($request->rest_mode)) {
                $post['rest_mode'] = 'off';
            }
    ...
    public function zoomSetting(request $request)
    {
        ...
        return back()->with('success', __('Zoom key succesfully added .'));
    }
    ...
    public function savePusherSettings(Request $request)
    {
        if (Auth::user()->type == 'company' || Auth::user()->type == 'super admin') {
            ...
            if ($envStripe) {
                return back()->with('success', __('Pusher successfully updated.'));
            } else {
    ...
    public function saveBusinessSettings(Request $request)
    {
        ...
        if (Auth::user()->type == 'company' || Auth::user()->type == 'super admin') {
            ...
            if ($request->company_logo_light) {
                ...
                // hash file before store
                $smallLogoName = md5($smallLogoName . time());
                $path          = $request->file('company_logo_light')->storeAs('uploads/logo', $smallLogoName);

                $company_logo_light = !empty($request->company_logo_light) ? $smallLogoName : 'logo-light.png';
    ...
    public function testSendMail(Request $request)
    {
        ...
        return back()->with('success', __('Email send Successfully.') . ((isset($smtp_error)) ? '<br> <span class="text-danger">' . $smtp_error . '</span>' : ''));
    }
    ...
    public function storeIp(Request $request)
    {
        if (Auth::user()->can('Manage Company Settings')) {
            ...
            return back()->with('success', __('IP successfully created.'));
        } else {
            return back()->with('error', __('Permission denied.'));
        }
    }
    ...
    public function updateIp(Request $request, $id)
    {
        if (Auth::user()->type == 'company' || Auth::user()->type == 'super admin') {
            ...
            return back()->with('success', __('IP successfully updated.'));
        } else {
            return back()->with('error', __('Permission denied.'));
        }
    }
    ...
    public function destroyIp($id)
    {
        if (Auth::user()->type == 'company' || Auth::user()->type == 'super admin') {
            ...
            return back()->with('success', __('IP successfully deleted.'));
        } else {
            return back()->with('error', __('Permission denied.'));
        }
    }
