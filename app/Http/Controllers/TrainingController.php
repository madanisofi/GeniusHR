...
class TrainingController extends Controller
{
    ...
    public function index()
    {
        if (Auth::user()->can('Manage Training')) {
            // Check if trainer is empty
            $trainer = Trainer::where('created_by', Auth::user()->creatorId())->get()->count();
            if ($trainer == 0) {
                return redirect()->route('trainer.index')->with('error', __('Please create trainer first.'));
            } else {
