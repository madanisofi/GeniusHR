...
class User extends Authenticatable
{
    ...
    {
        $employeetype = EmployeeType::where('id', '=', $employeetype_id)->first();
    ...
    public function lengthOfWork($doj)
    {
        $diff = \Carbon\Carbon::parse($doj)->diff(now());

        if ($diff->y > 0) {
            return $diff->y . ' ' . __('Year') . ' ' . $diff->m . ' ' . __('Month');
        } else if ($diff->y == 0 && $diff->m > 0) {
            return $diff->m . ' ' . __('Month');
        } else {
            return \Carbon\Carbon::parse($doj)->diffForHumans(null, true);
        }
    }
    ...
    {
        $check = \DB::table('roles')->where('name', $type)->where('created_by', '=', $this->creatorId())->get();
