        ...
        <form action="{{ route('password.confirm') }}" method="POST" class="pt-3 text-left needs-validation" novalidate="">
            ...
            <label class="d-block position-relative mb-3">
                ...
                <input type="password" id="password" name="password"
                    class="text-sm rounded-6 w-100 py-3 px-3 border text-grayDark @error('password') is-invalid @enderror">
                @error('password')
            ...
            <button type="submit" class="btn-primary px-4 py-2 text-xs"><span
                    class="d-block py-1">{{ __('Confirm Password') }}</span></button>
