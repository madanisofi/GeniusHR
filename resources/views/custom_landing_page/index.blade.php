@php
    ...
    $image_path=asset(url('uploads/custom_landing_page_image/'));
@endphp
    <!DOCTYPE html>
...
<head>
    ...
    <title>{{(Utility::getValByName('title_text')) ? Utility::getValByName('title_text') : config('app.name', 'GeniusHr')}}</title>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                @if (count($get_section) > 0)
                    @foreach ($get_section as $key => $value)
                        @if ($value->content == "" || $value->content == null)
                            <img src="{{asset('uploads/Landing_page_component_image')}}/{{$value->section_demo_image}}" onclick="set_section_page('{{$value->id}}')" id="component_modal_{{$value->id}}">
                        @endif
                    @endforeach
                @endif
            </div>
