    ...
    @forelse($employees as $employee)
        <div class="col-xl-3">
            <div class="card  text-center">
                ...
                <div class="card-body">
                    <div class="avatar">
                        <a href="{{ !empty($employee->user->avatar) ? asset(url('uploads/avatar')) . '/' . $employee->user->avatar : asset(url('uploads/avatar')) . '/avatar.png' }}" target="_blank">
                        <img src="{{ !empty($employee->user->avatar) ? asset(url('uploads/avatar')) . '/' . $employee->user->avatar : asset(url('uploads/avatar')) . '/avatar.png' }}"
                            ...
                            <div class="d-grid">
                                @can('Show Employee Profile')
                                    <a class="btn btn-outline-primary mx-5"
                                        href="{{ route('show.employee.profile', Crypt::encrypt($employee->id)) }}">{{ Auth::user()->employeeIdFormat($employee->employee_id) }}</a>
                                @else
                                    <a class="btn btn-outline-primary mx-5"
