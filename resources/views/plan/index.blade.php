            ...
            <div class="card price-card price-1 wow animate__fadeInUp" data-wow-delay="0.2s"
                ...
                <div class="card-body " style="height: 295px">
                    ...
                    <div class="row d-flex justify-content-between">
                        {{-- Temporary bypass --}}
                        {{-- @if (!empty($admin_payment_setting) && $admin_payment_setting['is_stripe_enabled'] == 'on') --}}
                            @can('Buy Plan')
                                @if ($plan->id != Auth::user()->plan && Auth::user()->type != 'super admin')
                                    <div class="col-8">
                                        @if (!$plan->price == 0)
                                            <div class="d-grid text-center">
                                                <a href="{{ route('stripe', Crypt::encrypt($plan->id)) }}"
                            ...
                            @if ($plan->id != 1)
                                <div class="col-3">
                                    @if (Auth::user()->requested_plan != $plan->id)
                                        <a href="{{ route('send.request', [Crypt::encrypt($plan->id)]) }}"
                                    ...
                                    @else
                                        <a href="{{ route('request.cancel', Auth::user()->id) }}"
                                            class="btn btn-danger btn-icon btn-sm" data-title="{{ __('Cancel Request') }}"
                                            data-bs-toggle="tooltip" data-bs-placement="top"
