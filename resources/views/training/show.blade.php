                        ...
                        <ul class="list-group list-group-flush">
                            <li class="list-group-item" style="border:0px;">
                                <div class="d-flex align-items-center">
                                    <a href="{{ !empty($training->employees) ? (!empty($training->employees->user->avatar) ? asset('uploads/avatar') . '/' . $training->employees->user->avatar : asset('uploads/avatar') . '/avatar.png') : asset('uploads/avatar') . '/avatar.png' }}"
                                        target="_blank">
                                        <img src="{{ !empty($training->employees) ? (!empty($training->employees->user->avatar) ? asset('uploads/avatar') . '/' . $training->employees->user->avatar : asset('uploads/avatar') . '/avatar.png') : asset('uploads/avatar') . '/avatar.png' }}"
                                            class="user-image-hr-prj ui-w-30 rounded-circle" width="50px" height="50px">
                                    </a>
                                    <div class="media-body px-2 text-sm">
                                        <a href="{{ route('employee.show', !empty($training->employees) ? Crypt::encrypt($training->employees->id) : 0) }}"
                                            class="text-dark">
